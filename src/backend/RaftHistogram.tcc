...
RaftHistogram<T, F>::RaftHistogram(F mean, F min, double binWidth) : raft::kernel(), mean(mean), min(min), binWidth(binWidth)
{
    input.addPort<T>("stdvvec");
    output.addPort<std::map<int, int>>("total");
}
template <typename T, typename F>
RaftHistogram<T, F>::RaftHistogram(const RaftHistogram &other) : raft::kernel(), mean(other.mean), min(other.min), binWidth(other.binWidth)
{
    input.addPort<T>("stdvvec");
    output.addPort<std::map<int, int>>("total");
}
